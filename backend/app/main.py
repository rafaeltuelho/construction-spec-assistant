...
async def construction_spec_assistant_exception_handler(
    request: Request, exc: ConstructionSpecAssistantError
) -> JSONResponse:
    """
...
async def validation_exception_handler(
    ...
    """
    logger.warning(
        f"Validation error: {exc}", extra={"path": request.url.path, "errors": exc.errors()}
    )
...
async def general_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    return JSONResponse(
        ...
        content={"error": "INTERNAL_SERVER_ERROR", "message": message, "details": {}},
    )
...
app.include_router(facts.router, prefix=settings.api_v1_prefix)

# Import comparison router
from app.api.v1 import comparison

app.include_router(
    comparison.router, prefix=f"{settings.api_v1_prefix}/comparison", tags=["comparison"]
)
