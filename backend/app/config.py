...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="Construction Spec Assistant", description="Application name")
    ...
    mongodb_database: str = Field(
        default="construction_spec_assistant", description="MongoDB database name"
    )
    ...
    qdrant_collection_name: str = Field(
        default="construction_docs", description="Qdrant collection name"
    )
    ...
    supported_file_types: List[str] = Field(
        default=[".pdf", ".docx", ".doc"], description="Supported file types"
    )
    upload_dir: str = Field(default="./data/uploads", description="Directory for uploaded files")

    # Docling Settings
    docling_use_ocr: bool = Field(default=True, description="Enable OCR for document parsing")
    docling_ocr_engine: str = Field(
        default="easyocr", description="OCR engine (easyocr or tesseract)"
    )
    ...
    fact_extraction_batch_size: int = Field(
        default=10, description="Number of chunks to process concurrently"
    )
    fact_confidence_threshold: float = Field(
        default=0.5, description="Minimum confidence for extracted facts"
    )

    # Retrieval Settings
    retrieval_top_k: int = Field(default=5, description="Number of documents to retrieve")
    dense_weight: float = Field(default=0.5, description="Weight for dense retrieval (0-1)")
    sparse_weight: float = Field(default=0.5, description="Weight for sparse retrieval (0-1)")
    embedding_model: str = Field(
        default="BAAI/bge-small-en-v1.5", description="Embedding model for dense retrieval"
    )
